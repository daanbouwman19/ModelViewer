...
## 2025-02-19 - Sequential I/O in Worker Threads

**Learning:** Found an N+1 I/O bottleneck in `database-worker.ts` where `fs.stat` was called sequentially for thousands of files.
**Action:** Use `Promise.all` with batching (e.g., chunk size 50) to parallelize file system operations without exhausting file descriptors.
