...
const { initDatabase, recordMediaView, getMediaViewCounts, cacheModels, getCachedModels, closeDatabase } = require('./database.js');
const { performFullMediaScan } = require('./media-scanner.js');
const { startLocalServer, stopLocalServer, getServerPort, getMimeType: resolveMimeType } = require('./local-server.js'); // Import stopLocalServer
...
async function getModelsFromCacheOrDisk() {
    ...
    if (models && models.length > 0) { // Check if cache is not empty
        console.log('[main.js] Loaded models from cache.');
        return models;
    }
    console.log('[main.js] No file index cache found or cache empty. Scanning disk...');
