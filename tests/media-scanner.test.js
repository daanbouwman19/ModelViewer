...
describe('media-scanner.js', () => {
    beforeAll(() => {
        ...
        createDummyFile(path.join(TEST_MEDIA_DIR, 'model2', 'subfolder', 'image3.jpeg'));
    ...
    describe('findAllMediaFiles', () => {
        ...
        it('should find files in subdirectories', () => {
            ...
            expect(files).toEqual(expect.arrayContaining([
                ...
                { name: 'image3.jpeg', path: path.join(model2Path, 'subfolder', 'image3.jpeg') },
            ]));
        });
        ...
        it('should only include files with supported extensions', () => {
            ...
            const unsupportedFilePath = path.join(TEST_MEDIA_DIR, 'model1', 'unsupported.abc');
    ...
    describe('performFullMediaScan', () => {
        it('should perform a full scan and identify models with their media files', async () => {
            ...
            expect(model1.textures).toEqual(expect.arrayContaining([
                { name: 'image1.png', path: path.join(TEST_MEDIA_DIR, 'model1', 'image1.png') },
                { name: 'video1.mp4', path: path.join(TEST_MEDIA_DIR, 'model1', 'video1.mp4') },
            ]));

            const model2 = models.find(m => m.name === 'model2');
            expect(model2).toBeDefined();
            expect(model2.textures).toHaveLength(2);
            expect(model2.textures).toEqual(expect.arrayContaining([
                { name: 'image2.jpg', path: path.join(TEST_MEDIA_DIR, 'model2', 'image2.jpg') },
                { name: 'image3.jpeg', path: path.join(TEST_MEDIA_DIR, 'model2', 'subfolder', 'image3.jpeg') },
            ]));
        });

        it('should return an empty array if the base media directory does not exist', async () => {
            const nonExistentBaseDir = path.join(__dirname, 'non_existent_base_media_dir');
