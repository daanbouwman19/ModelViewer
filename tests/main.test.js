...
describe('Main Process', () => {
    ...
    describe('IPC Handlers', () => {
        ...
        it('should register all IPC handlers', () => {
            const registeredHandlers = ipcMain.handle.mock.calls.map(call => call[0]);
        ...
        describe('load-file-as-data-url', () => {
            it('should return an error if the file does not exist', async () => {
                ...
                expect(result).toEqual({ type: 'error', message: 'File does not exist.' });
            });
            ...
            it('should return an http-url for a large video file when the server is running', async () => {
                ...
                fs.statSync.mockReturnValue({ size: (MAX_DATA_URL_SIZE_MB + 1) * 1024 * 1024 });
                ...
                expect(result).toEqual({ type: 'http-url', url: 'http://localhost:8080/large-video.mp4' });
            });
            ...
            it('should return an error for a large video file when the server is not ready', async () => {
                ...
                const handler = getIpcHandler('load-file-as-data-url');
                const result = await handler(null, 'large-video.mp4');
                expect(result).toEqual({ type: 'error', message: 'Local server not ready to stream large video.' });
            });
        ...
        describe('Other IPC Handlers', () => {
            ...
            it('should get models from cache and return with view counts', async () => {
                const { getCachedModels, getMediaViewCounts } = require('../main/database.js');
            ...
            it('should re-index and return models with view counts', async () => {
                ...
                const { cacheModels, getMediaViewCounts } = require('../main/database.js');
