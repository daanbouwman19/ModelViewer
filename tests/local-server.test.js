...
describe('local-server.js', () => {
  ...
  describe('Server Start and Stop', () => {
    it('should start the server on a random available port and then stop it', (done) => {
      ...
      startLocalServer(() => {
        ...
        http
          .get(`http://127.0.0.1:${port}/${encodedFilePath}`, (res) => {
  ...
  describe('File Serving', () => {
    ...
    it('should serve an image file with correct Content-Type and Content-Length', (done) => {
      const encodedFilePath = encodeURIComponent(TEST_IMAGE_FILE);
      http
        .get(`http://127.0.0.1:${serverPort}/${encodedFilePath}`, (res) => {
          expect(res.statusCode).toBe(200);
          expect(res.headers['content-type']).toBe(
            getMimeType(TEST_IMAGE_FILE),
          );
          expect(res.headers['content-length']).toBe(
            String(fs.statSync(TEST_IMAGE_FILE).size),
          );
    ...
    it('should serve a video file with correct Content-Type and Content-Length', (done) => {
      ...
      http
        .get(`http://127.0.0.1:${serverPort}/${encodedFilePath}`, (res) => {
          expect(res.statusCode).toBe(200);
          expect(res.headers['content-type']).toBe(
            getMimeType(TEST_VIDEO_FILE),
          );
          expect(res.headers['content-length']).toBe(
            String(fs.statSync(TEST_VIDEO_FILE).size),
          );
          expect(res.headers['accept-ranges']).toBe('bytes');
          res.resume();
          done();
    ...
    it('should return 404 for a non-existent file', (done) => {
      const nonExistentFile = encodeURIComponent(
        path.join(TEST_DIR, 'non_existent_file.txt'),
      );
      http
        .get(`http://127.0.0.1:${serverPort}/${nonExistentFile}`, (res) => {
    ...
    it('should handle file paths with spaces', (done) => {
      const testFileWithSpace = path.join(
        TEST_DIR,
      ...
      http
        .get(`http://127.0.0.1:${serverPort}${urlPath}`, (res) => {
          expect(res.statusCode).toBe(200);
          expect(res.headers['content-type']).toBe(
            getMimeType(testFileWithSpace),
          );
          expect(res.headers['content-length']).toBe(
            String(fs.statSync(testFileWithSpace).size),
          );
          res.resume();
