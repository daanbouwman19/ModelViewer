...
describe('Media Routes Coverage', () => {
  ...
  beforeEach(() => {
    ...
    app.use(mediaRoutes.createMediaRoutes({
    ...
    // Default Security
    vi.mocked(security.authorizeFilePath).mockResolvedValue({ isAllowed: true } as any);
    vi.mocked(security.filterAuthorizedPaths).mockImplementation(async (paths) => paths);
    vi.mocked(mediaHandler.validateFileAccess).mockResolvedValue({ success: true, path: '/file.mp4' });
  });
  ...
  describe('Validation & Errors', () => {
      ...
      it('POST /api/media/view access denied', async () => {
          vi.mocked(security.authorizeFilePath).mockResolvedValue({ isAllowed: false, message: 'Denied' } as any);
          const res = await request(app).post('/api/media/view').send({ filePath: '/secret.mp4' });
          expect(res.status).toBe(403);
          expect(res.text).toBe('Denied');
      });

      it('POST /api/media/views invalid body', async () => {
          const res = await request(app).post('/api/media/views').send({ filePaths: 'not-array' });
          expect(res.status).toBe(400);
      });

      it('POST /api/media/views exceeds batch limit', async () => {
          const paths = Array(MAX_API_BATCH_SIZE + 1).fill('/path.mp4');
          const res = await request(app).post('/api/media/views').send({ filePaths: paths });
          expect(res.status).toBe(400);
          expect(res.text).toContain('Batch size exceeds limit');
      });

      it('POST /api/media/rate missing arguments', async () => {
          const res = await request(app).post('/api/media/rate').send({ filePath: '/f.mp4' }); // missing rating
          expect(res.status).toBe(400);
      });

      it('POST /api/media/metadata missing arguments', async () => {
          const res = await request(app).post('/api/media/metadata').send({ filePath: '/f.mp4' }); // missing metadata
          expect(res.status).toBe(400);
      });
  ...
  describe('Streaming Logic', () => {
      ...
      it('GET /api/stream access denied', async () => {
          vi.mocked(mediaHandler.validateFileAccess).mockResolvedValue({ success: false, statusCode: 403, error: 'Denied' });
      ...
      it('GET /api/stream transcode ffmpeg missing', async () => {
          ...
          appNoFfmpeg.use(mediaRoutes.createMediaRoutes({
            limiters: mockLimiters as any,
            mediaHandler: mockMediaHandler,
            transcodeState: { current: 0 },
            ffmpegPath: null
          ...
          const res = await request(appNoFfmpeg).get('/api/stream?file=/v.mp4&transcode=true');
  ...
  describe('Serve Route Errors', () => {
      ...
      it('GET /api/serve generic error', async () => {
          vi.mocked(mediaHandler.validateFileAccess).mockRejectedValue(new Error('Random Error'));
