...
describe('System Routes Coverage', () => {
  ...
  describe('Validation Edge Cases', () => {
      ...
      it('PUT /api/smart-playlists/:id invalid id', async () => {
          const res = await request(app).put('/api/smart-playlists/abc').send({ name: 'n', criteria: 'c' });
          expect(res.status).toBe(400);
      });
      ...
      it('POST /api/directories invalid path type', async () => {
          const res = await request(app).post('/api/directories').send({ path: 123 });
          expect(res.status).toBe(400);
      });

      it('POST /api/directories security validation fail', async () => {
          vi.mocked(security.validateInput).mockReturnValue({ isValid: false, message: 'Bad Input' });
          const res = await request(app).post('/api/directories').send({ path: '/valid/but/bad/chars' });
          expect(res.status).toBe(400);
  ...
  describe('File System Routes', () => {
      ...
      it('GET /api/fs/ls security validation fail', async () => {
          vi.mocked(security.validateInput).mockReturnValue({ isValid: false, message: 'Bad' });
  ...
  describe('Directory State', () => {
      it('PUT /api/directories/active missing path', async () => {
          const res = await request(app).put('/api/directories/active').send({ isActive: true });
          expect(res.status).toBe(400);
      });
  });
});
