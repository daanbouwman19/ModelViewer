...
import { serveTranscodedStream, serveRawStream } from '../../src/core/media-handler';
...
describe('media-handler unit tests', () => {
  ...
  describe('serveTranscodedStream', () => {
    it('spawns ffmpeg with correct arguments for local file', async () => {
      ...
      expect(mockSpawn).toHaveBeenCalledWith(ffmpegPath, expect.arrayContaining([
        '-i', sourceInput,
        '-f', 'mp4',
        'pipe:1'
      ]));
    ...
    it('adds start time argument if provided', async () => {
      ...
      await serveTranscodedStream(req, res, mockMediaSource, ffmpegPath, startTime);
    ...
    it('propagates errors from getFFmpegInput', async () => {
        vi.mocked(mockMediaSource.getFFmpegInput).mockRejectedValue(new Error('Access denied'));

        await expect(serveTranscodedStream(req, res, mockMediaSource, '/bin/ffmpeg', null))
  ...
  describe('serveRawStream', () => {
      it('pipes stream with correct headers for full content', async () => {
          ...
          vi.mocked(mockMediaSource.getStream).mockResolvedValue({ stream: mockStream, length: 1000 });

          await serveRawStream(req, res, mockMediaSource);

          expect(mockMediaSource.getStream).toHaveBeenCalledWith({ start: 0, end: 999 });
      ...
      it('handles Range requests correctly', async () => {
        ...
        const mockStream = new PassThrough();
        vi.mocked(mockMediaSource.getStream).mockResolvedValue({ stream: mockStream, length: 100 });

        await serveRawStream(req, res, mockMediaSource);

        expect(mockMediaSource.getStream).toHaveBeenCalledWith({ start: 100, end: 199 });
        expect(res.writeHead).toHaveBeenCalledWith(206, expect.objectContaining({
    ...
    it('propagates errors from getStream', async () => {
        vi.mocked(mockMediaSource.getSize).mockResolvedValue(1000);
        vi.mocked(mockMediaSource.getMimeType).mockResolvedValue('video/mp4');
        vi.mocked(mockMediaSource.getStream).mockRejectedValue(new Error('Access denied'));

        await expect(serveRawStream(req, res, mockMediaSource))
            .rejects.toThrow('Access denied');
