...
import { serveHlsMaster, serveHlsPlaylist, serveHlsSegment } from '../../src/core/hls-handler.ts';
...
describe('hls-handler', () => {
  ...
  describe('serveHlsMaster', () => {
    it('should serve master playlist if access is granted', async () => {
      vi.mocked(validateFileAccess).mockResolvedValue({ success: true, path: '/resolved/video.mp4' });
      ...
      expect(res.send).toHaveBeenCalledWith(expect.stringContaining('playlist.m3u8?file=%2Fpath%2Fto%2Fvideo.mp4'));
    });

    it('should handle access denied', async () => {
      vi.mocked(validateFileAccess).mockResolvedValue({ success: false, statusCode: 403, error: 'Access denied' });
  ...
  describe('serveHlsPlaylist', () => {
    it('should serve playlist if session exists', async () => {
      ...
      expect(res.set).toHaveBeenCalledWith('Content-Type', 'application/vnd.apple.mpegurl');
      expect(res.send).toHaveBeenCalledWith(expect.stringContaining('segment_000.ts?file=%2Fpath%2Fto%2Fvideo.mp4'));
      expect(mockHlsManager.touchSession).toHaveBeenCalledWith('mock-session-id');
    });
    ...
    it('should throw error if session dir not found', async () => {
       ...
       expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Playlist error'), expect.any(Error));
  ...
  describe('serveHlsSegment', () => {
    it('should serve segment if valid name and session exists', async () => {
       ...
       expect(res.sendFile).toHaveBeenCalledWith(path.join('/tmp/hls/mock-session-id', 'segment_001.ts'));
    });

    it('should reject invalid segment names (Security)', async () => {
       vi.mocked(validateFileAccess).mockResolvedValue({ success: true, path: '/resolved/video.mp4' });

       const invalidNames = ['../passwd', 'segment_001.ts.bak', 'segment_abc.ts', 'other.txt', 'segment_1.ts/'];
    ...
    it('should return 404 if session expired', async () => {
       ...
       expect(res.send).toHaveBeenCalledWith('Segment not found (Session expired)');
    });
  });
});
