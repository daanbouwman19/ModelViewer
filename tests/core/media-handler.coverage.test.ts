...
import { getDriveFileMetadata, getDriveFileStream } from '../../src/main/google-drive-service';
...
vi.mock('fs', () => ({ default: { promises: mockFsPromises }, promises: mockFsPromises }));
...
describe('media-handler coverage', () => {
  ...
  describe('generateFileUrl', () => {
    it('should return error for large Drive file if serverPort is 0', async () => {
      (getDriveFileMetadata as any).mockResolvedValue({ size: 10 * 1024 * 1024, mimeType: 'video/mp4' });
    ...
    it('should return error for large local file if serverPort is 0', async () => {
      ...
      const result = await generateFileUrl('/local/large.mp4', { serverPort: 0 });
      expect(result).toEqual({
        type: 'error',
    ...
    it('should return http-url for large local file if serverPort > 0', async () => {
      ...
      const result = await generateFileUrl('/local/large.mp4', { serverPort: 3000 });
      expect(result).toEqual({
        type: 'http-url',
    ...
    it('should handle fs errors gracefully', async () => {
       ...
       const result = await generateFileUrl('/local/file.mp4', { serverPort: 3000 });
       expect(result).toEqual({
         type: 'error',
         message: 'FS Error'
       });
    ...
    it('should return data-url for small Drive file', async () => {
      (getDriveFileMetadata as any).mockResolvedValue({ size: 100, mimeType: 'video/mp4' });
      const mockStream = {
      ...
      const result = await generateFileUrl('gdrive://123', { serverPort: 3000 });
    ...
    it('should return data-url for small local file', async () => {
      ...
      const result = await generateFileUrl('/local/small.mp4', { serverPort: 3000 });
      expect(result.type).toBe('data-url');
    });
  });
  ...
  describe('openMediaInVlc', () => {
    ...
    it('should prepare stream url for Drive file', async () => {
        ...
        expect(mockSpawn).toHaveBeenCalledWith('vlc',
