...
import { isDrivePath, getVlcPath } from './media-utils.ts';
...
export async function openMediaInVlc(
  ...
  try {
    ...
    child.on('error', (err) => {
      console.error('[vlc-handler] Error launching VLC (async):', err);
    });

    child.unref();
    return { success: true };
  } catch (error: unknown) {
    console.error('[vlc-handler] Error launching VLC:', error);
    return {
      success: false,
